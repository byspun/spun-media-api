...
export async function getVidlinkStreams(input: MovieProviderInput, tmdbApiKey: string): Promise<RawStream[]> {
  const payload = await fetchJson<any>(`https://vidlink.pro/api/b/movie/${input.tmdb_id}?api_key=${encodeURIComponent(tmdbApiKey)}`, { headers: { Referer: 'https://vidlink.pro/', Origin: 'https://vidlink.pro' }, timeout: 20_000 });
