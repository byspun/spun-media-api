...
export async function getDaratechStreams(input: MovieProviderInput, config: DaratechConfig): Promise<RawStream[]> {
  ...
  const search = await fetchJson<any>(`${base}/search/movies?q=${encodeURIComponent(input.title)}`, { headers, timeout: 15_000 }); const items = Array.isArray(search?.items) ? search.items : Array.isArray(search?.results) ? search.results : []; const index = findBestMatch(items.map((item: any) => ({ title: String(item.title ?? ''), year: Number(item.year) || null })), input, 40); if (index < 0) return [];
  const subjectId = items[index]?.subjectId ?? items[index]?.id; if (!subjectId) return []; const payload = await fetchJson<any>(`${base}/movies/${encodeURIComponent(String(subjectId))}/stream`, { headers, timeout: 20_000 }); const qualities = Array.isArray(payload?.qualities) ? payload.qualities : []; const subtitles = (Array.isArray(payload?.subtitles) ? payload.subtitles : []).map(subtitle).filter(Boolean) as RawSubtitle[];
