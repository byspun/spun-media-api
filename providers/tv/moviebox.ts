...
export async function getMovieboxStreams(input: TvProviderInput, _tmdbApiKey: string): Promise<RawStream[]> {
  ...
  const groups = Array.isArray(search?.data?.data?.results) ? search.data.data.results : []; const subject = bestSubject(groups.flatMap((group: any) => Array.isArray(group.subjects) ? group.subjects : []), input); if (!subject) return [];
  ...
  for (const item of subjects) { const result = await request('GET', `${API_BASE}/wefeed-mobile-bff/subject-api/play-info?subjectId=${encodeURIComponent(item.id)}&se=${input.season}&ep=${input.episode}`); const values = Array.isArray(result?.data?.data?.streams) ? result.data.data.streams : []; for (const value of values) { if (!isSafeHttpUrl(value.url)) continue; const streamId = String(value.id ?? `${item.id}|${input.season}|${input.episode}`); subtitles.push(...await getCaptions(item.id, streamId)); streams.push({ url: value.url, format: detectFormat(value.url, value.format) as any, quality: normalizeQuality(value.resolutions ?? value.quality), audio: item.lang, provider: 'moviebox', headers: { Referer: API_BASE, ...(value.signCookie ? { Cookie: value.signCookie } : {}) } }); } }
